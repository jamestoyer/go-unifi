...
func (dst *SettingRadioAi) UnmarshalJSON(b []byte) error {
	...
	aux := &struct {
		Channels6E *[]emptyStringInt `json:"channels_6e,omitempty"`
		ChannelsNa *[]emptyStringInt `json:"channels_na,omitempty"`
		ChannelsNg *[]emptyStringInt `json:"channels_ng,omitempty"`
		HtModesNa  *[]emptyStringInt `json:"ht_modes_na,omitempty"`
		HtModesNg  *[]emptyStringInt `json:"ht_modes_ng,omitempty"`
	...
	if aux.Channels6E != nil {
		tmp := make([]int, len(*aux.Channels6E))
		for i, v := range *aux.Channels6E {
			tmp[i] = int(v)
		}
		dst.Channels6E = &tmp
	}
	if aux.ChannelsNa != nil {
		tmp := make([]int, len(*aux.ChannelsNa))
		for i, v := range *aux.ChannelsNa {
			tmp[i] = int(v)
		}
		dst.ChannelsNa = &tmp
	}
	if aux.ChannelsNg != nil {
		tmp := make([]int, len(*aux.ChannelsNg))
		for i, v := range *aux.ChannelsNg {
			tmp[i] = int(v)
		}
		dst.ChannelsNg = &tmp
	}
	if aux.HtModesNa != nil {
		tmp := make([]int, len(*aux.HtModesNa))
		for i, v := range *aux.HtModesNa {
			tmp[i] = int(v)
		}
		dst.HtModesNa = &tmp
	}
	if aux.HtModesNg != nil {
		tmp := make([]int, len(*aux.HtModesNg))
